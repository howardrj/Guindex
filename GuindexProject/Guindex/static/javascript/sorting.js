(function () {
    ...
    var guindex_table = document.getElementById('guindex_table');
    ...
    function sortRows (evt)
    {
        ...
        var first_row = guindex_table.rows[0]; 
        ...
        for (var i = 0; i < guindex_table.rows.length - 2; i++)
        {
            table_rows_content[i] = guindex_table.rows[i + 1].cells[column_index].getElementsByClassName('payload')[0].innerHTML;
        }
        ...
        if (evt.target.innerHTML.indexOf(DOWN_ARROW) != -1) // Column is in descending order
        {
            ...
            resetOtherColumnHeaders(evt, guindex_table);
        }
        ...
        var table_rows_copy = Array.from(guindex_table.rows); // Create copy of current table rows
        ...
        var last_row = guindex_table.rows[guindex_table.rows.length - 1];

        while(guindex_table.rows[1]) {
            guindex_table.rows[1].parentNode.removeChild(guindex_table.rows[1]);   
        }

        // Append sorted rows to table    
        for (var i = 0; i < updated_table_rows.length; i++)
        {
            $('#' + guindex_table.id).append(updated_table_rows[i]);
        }

        // Append last row
        $('#' + guindex_table.id).append(last_row);
    }
